...
Добро пожаловать! Эта книга научит вас языку программирования Rust. Rust современный язык программирования с упором на безопасность и скорость. Эти задачи решаются путем безопасного использования памяти без сборщика мусора.

"Язык программирования Rust" делится на три части, перемещаться по которым, вы можете по меню слева.

##Основной

Этот раздел введение в основной синтаксис и семантику Rust. Он содержит отдельные разделы по каждой части синтаксиса в Rust, и завершается в небольшим проектом: игрой в отгадки.

После прочтения раздела "Основной", у вас будут хорошая основа, вы узнаете больше о Rust, и сможете написать очень простые программы.

##Средний

Этот раздел содержит отдельные главы, которые являются самодостаточными. Они сосредоточены на конкретных темах и могут быть прочитаны в любом порядке.

После прочтения раздела "Средний", вы будете иметь четкое понимание языка Rust, и будете в состоянии понять большую часть кода Rust и писать более сложные программы.

##Дополнительный

Аналогичным образом как и в разделе "Средний", он полон индивидуальных, для глубоко погружения глав, которые стоят по одной, и могут быть прочитаны в любом порядке. Эти главы посвящены наиболее сложным функцияи, а также некоторым вещам, которые будут доступны только в будущих версиях Rust.

После прочтения раздела "Дополнительный", вы будете экспертом Rust!

# 1 Установка Rust #

Первым шагом к использованию Rust, его установка! Есть несколько способов, чтобы установить Rust, но проще всего использовать скрипт rustup. Если вы находитесь на Linux или Mac, все, что вам нужно сделать, это (Заметим, что вам не нужно вводить в $ S, они просто указывают на начало каждой команды):

    $ curl -L https://static.rust-lang.org/rustup.sh | sudo sh

Если вы беспокоитесь о потенциальной безопасности использования curl | sudo sh, пожалуйста, продолжайте читать и увидите наш отказ от ответственности ниже. И не стесняйтесь использовать двухступенчатую версию установки и изучить наш установочный скрипт:

    $ curl -L https://static.rust-lang.org/rustup.sh -O
    $ sudo sh rustup.sh

Если вы находитесь на Windows, загрузите либо 32-битную версию программы установки или 64-битную версию программы установки и запустите его.

Если вы решили, что не хотите больше использовать Rust, нам будет немного грустно, но это нормально. Не каждый язык программирования отлично подходит для всех. Просто запустите скрипт деинсталляции:

    $ sudo /usr/local/lib/rustlib/uninstall.sh

Если вы использовали установщик Windows, просто повторно запустить .exe, это даст вам возможность удаления.

Вы можете запускать этот скрипт в любое время вы хотите обновить Rust. С которым это часто происходит, в данный момент. Rust все еще предварительная 1,0, и так люди считают, что вы используете совсем недавно Rust.

Это подводит меня к одной другой точке: некоторые люди, и несколько справедливо, весьма расстраиваются, когда мы говорим вам, чтобы curl | sudo sh. И так должно быть! В основном, когда вы сделаете это, вы веря, что хорошие люди, которые утверждают, что Rust не собирается взламывать ваш компьютер и делать плохие вещи. Это хороший инстинкт! Если вы один из тех людей, пожалуйста, ознакомьтесь с документацией на установку из источника Rust или официальных бинарных файлов для загрузки. И мы обещаем, что этот метод не будет постоянным способом установки Rust: это просто самый простой способ, чтобы люди могли обновиться, пока Rust в состоянии альфа.

О, мы должны упомянуть также официально поддерживаемые платформы:

Windows (7, 8, Server 2008 R2)
Linux (2.6.18 или более поздней версии, различные дистрибутивы), x86 и x86-64
OSX 10.7 (Lion) или выше, x86 и x86-64
Мы активно тестируем Rust на этих платформах, и нескольких других, так же, как и Android. Но это те, скорее всего похоже на работу, так как максимально тестируется.

Наконец, комментарий о Windows. Rust считает Windows площадкой первого класса, на релизе, но если быть честными, опыт Windows не обладает таким внедрением опыта как в Linux / OS X. Мы работаем над этим! Если что-то не работает, это ошибка. Пожалуйста, дайте нам знать, если это произойдет. Каждая фиксация проверяется на Windows, так же, как любой другой платформе.

Если вы установите Rust, вы можете открыть оболочку и ввести следующее:

    $ rustc --version

Вы должны увидеть номер версии, хэш, дату совершения и дату сборки:
 
    rustc 1.0.0-nightly (f11f3e7ba 2015-01-04) (built 2015-01-06)

Если все так и есть, Rust был успешно установлен! Поздравляю!

Если нет, есть ряд мест, где можно получить помощь.Проще всего #rust IRC канал на irc.mozilla.org, что вы можете получить доступ через Mibbit. Нажмите на эту ссылку, и вы будете общаться в чате с другими Rustaceans (глупое прозвище, которым мы называем себя), и мы сможем помочь вам. Другие большие ресурсы включают в себя /r/rust subreddit и  Stack Overflow.
